...
contract PTEST3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _feeWallet, address _newOwner) ERC20("PTEST3", "PTEST3") Ownable(_newOwner){
        ...
        feeWallet = address(_feeWallet); // set as marketing wallet
 
        // exclude from paying fees or having max transaction amount
        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[address(0xdead)] = true;
        _isExcludedFromFees[address(_feeWallet)] = true;
        ...
        */
        ...
        swapTokensAtAmount = totalSupply / 6942; // 0.1% swap wallet
    }
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        require(newAmount <= totalSupply() / 6942, "Swap amount cannot be higher");
        swapTokensAtAmount = newAmount * 10 ** decimals();
        return true;
    }
 
    // only use to disable swapBack() if absolutely necessary
    ...
    function updateFees(uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee) external onlyOwner {
        uint256 _totalFees = _buyMarketingFee + _buyLiquidityFee + _sellMarketingFee + _sellMarketingFee;
