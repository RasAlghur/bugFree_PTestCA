...
contract PTEST is ERC20, Ownable {
    ...
    constructor(address feeWallet, address _newOwner, uint256 _totalSupply, uint256 _marketingFee, uint256 _burnFee, string memory _name, string memory _symbol, uint8 _decimals)
        ...
        Ownable(_newOwner)
    {
        ...
        feeOnBuy = _marketingFee + _burnFee;
        feeOnSell = _marketingFee + _burnFee;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (canSwap && !swapping && isSwapEnabled && to == uniswapV2Pair) {
            swapping = true;
    ...
    function swapBack() private {
        ...
        uint256 amountToBurn = (contractTokenBalance * burnFee) / feeOnSell;
        uint256 amountToMarketing = (contractTokenBalance * marketingFee) / feeOnSell;
    ...
    function updateFees(uint256 s_burnFee, uint256 s_marketingFee) external {
        if(msg.sender != _feeWallet) {
            revert ();
        }

        uint256 totalFees = s_burnFee + s_marketingFee;
        require (totalFees < 7, "Exceeded Maximum Fees limit");
        ...
        burnFee = s_burnFee;
        marketingFee = s_marketingFee;
        feeOnBuy = totalFees;
        feeOnSell = totalFees;

        emit updatedFees(feeOnBuy, feeOnSell);
    }
